...
public class $08_OrderTxDetect_02 {
    public static void main(String[] args) throws Exception {
        ...
        //连接两条流，如果使用连接条件进行匹配，要对 连接条件 进行keyby，避免多并行度的影响
        KeyedStream<OrderEvent, String> orderKS = orderDS.keyBy(order -> order.getTxId());
        KeyedStream<TxEvent, String> tsKS = txDS.keyBy(tx -> tx.getTxId());


        // 关联两条流，使用 connect
        ConnectedStreams<OrderEvent, TxEvent> orderTxCS = orderKS.connect(tsKS);

        // 使用 ProcessFunction
        SingleOutputStreamOperator<String> redultDS = orderTxCS.process(new OrderTxDetectFunction());

...
// 实时对账
public class Flink02_Case_OrderTxDetectWithIntervalJoin {
    public static void main(String[] args) throws Exception {
        ...
        SingleOutputStreamOperator<TxEvent> txDS = env
                .readTextFile("basic/input/ReceiptLog.csv")
                .map(
        ...
        orderDS.keyBy(order -> order.getTxId())
                ...
                .process(new ProcessJoinFunction<OrderEvent, TxEvent, String>() {
                    ...
                    public void processElement(OrderEvent left, TxEvent right, Context ctx, Collector<String> out) throws Exception {
                        // 每次进来一组，（基准数据 跟 另外一个条流里 界限范围内的数据，相同 key 的数据 轮流发生关系）
